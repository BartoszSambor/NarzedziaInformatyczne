...
float dotScalar(int ix, int iy, float x, float y, int seed) {
    float xFraction = x - (float)ix;
    float yFraction = y - (float)iy;

    float xRandPoint = (float)cos(randomNumber(ix, iy, seed));
    float yRandPoint = (float)sin(randomNumber(ix, iy, seed));

    return (xFraction*xRandPoint + yFraction*yRandPoint);
}

float noiseSimple(float x, float y, int seed){
    seed %= 256;
    int xFloor = (int) x;
    int yFloor = (int) y;
...
float noiseSimpleRecursive(int x, int y, float density, int depth, int seed){
    float result = 0;
    float value = 1.0f;
    for(int i = 0; i < depth; ++i){
        float xFloat = (float)x / density;
        float yFloat = (float)y / density;
        result += (noiseSimple(xFloat, yFloat, seed)-128) * value;
        density *= 2;
        value /= 2;
    }
...
float noiseComplex(float x, float y, int seed){
    int xFloor = (int)x;
    int xCeil = xFloor + 1;
    int vFloor = (int)y;
    int yCeil = vFloor + 1;
    ...
    float xFraction = x - (float)xFloor;
    float yFraction = y - (float)vFloor;

    float corner00 = dotScalar(xFloor, vFloor, x, y, seed);
    float corner01 = dotScalar(xFloor, yCeil, x, y, seed);
    float corner10 = dotScalar(xCeil, vFloor, x, y, seed);
    float corner11 = dotScalar(xCeil, yCeil, x, y, seed);

    float y1Smooth = interpolate(corner00, corner10, xFraction);
    float y2Smooth = interpolate(corner01, corner11, xFraction);
    float xySmooth  = interpolate(y1Smooth, y2Smooth, yFraction);
    return xySmooth;
}

float noiseComplexRecursive(int x, int y, float density, int depth, int seed){
    float result = 0;
    float value = 1.0f;
    for(int i = 0; i < depth; ++i){
        float xFloat = (float)x / density;
        float yFloat = (float)y / density;
        result += noiseComplex(xFloat, yFloat, seed) * value;
        density *= 2;
        value /= 2;
    }
